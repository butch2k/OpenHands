    ...
    async def upload_file(
        ...
        try:
            ...
            if recursive or safe_filename.endswith('.zip'):
                ...
                # Extract the zip file with zip-slip protection
                dest_realpath = os.path.realpath(full_dest_path)
                with ZipFile(zip_path, 'r') as zf:
                    ...
                    zf.extractall(full_dest_path)
                os.remove(zip_path)  # Remove the zip file after extraction

                logger.debug(
                    f'Uploaded file {safe_filename} and extracted to {destination}'
                )
            else:
                ...
                with open(file_path, 'wb') as buffer:
                    shutil.copyfileobj(file.file, buffer)
                logger.debug(f'Uploaded file {safe_filename} to {destination}')
